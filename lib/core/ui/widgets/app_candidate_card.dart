...
final class AppCandidateCard extends StatelessWidget {
  const AppCandidateCard({required this.candidate, required this.privatePhotoLabel, super.key});
              ...
              child: Stack(
                ...
                children: [
                  ImageFiltered(
                    ...
                    child: Transform.scale(scale: 1.18, child: _buildImage(candidate)),
                  ),
  ...
  Widget _buildImage(AppCandidateCardData candidate) {
    ...
    final fallbackWidget = defaultAsset.image(fit: BoxFit.cover, filterQuality: FilterQuality.medium, excludeFromSemantics: true);
...
final class _PrivatePhotoPill extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return DecoratedBox(
      decoration: BoxDecoration(color: AppColors.text, borderRadius: BorderRadius.circular(AppRadius.full)),
      child: Padding(
